...
final class NonEmptyPath(
  ...
  override def head = props.head
  
  override def tail = new NonEmptyPath(props.tail)
  
  override def step(p: Property): Path =
    new NonEmptyPath(props :+ p)

  override def ++(other: Path): Path =
    new NonEmptyPath(props ++ other.props)

  override def last: Property = props.last
  ...
  def canEqual(other: Any) = {
    other.isInstanceOf[org.beandiff.core.model.NonEmptyPath]
  }

  override def equals(other: Any) = {
    other match {
      case that: org.beandiff.core.model.NonEmptyPath => that.canEqual(NonEmptyPath.this) && props == that.props
      case _ => false
    }
  }
